...
class MyDummyComponentLauncher(InProcessComponentLauncher):
  ...
  def _run_executor(self, execution_id: int,
    ...
    if component_id not in self.dummy_dict.keys():
      ...
      dummy_class = self.dummy_dict[component_id]
      executor = dummy_class(component_id, self._record_dir, executor_context)
