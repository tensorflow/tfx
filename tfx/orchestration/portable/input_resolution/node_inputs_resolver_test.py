...
class NodeInputsResolverTest(tf.test.TestCase):
  ...
  def testMixedInput_Coalesce(self):
    a1, a2, a3 = self.create_artifacts(3)
    x1, x2, x3 = [pipeline_pb2.InputSpec() for _ in range(3)]
    for input_spec, output_key in zip((x1, x2, x3), ('x1', 'x2', 'x3')):
      channel = input_spec.channels.add()
      channel.artifact_query.type.name = 'MyArtifact'
      channel.output_key = output_key

    x4 = self.parse_input_spec("""
      mixed_inputs {
        method: COALESCE
        input_keys: "x1"
        input_keys: "x2"
    ...
    for mock_combination in [
      ...
      with self.subTest(mock_combination=mock_combination):
        x1_mock, x2_mock, x3_mock = mock_combination
        self.mock_channel_resolution(x1, x1_mock)
        self.mock_channel_resolution(x2, x2_mock)
        self.mock_channel_resolution(x3, x3_mock)
        # x4 = COALESCE(x1, x2, x3)
        x4_expected = x1_mock or x2_mock or x3_mock

        result = node_inputs_resolver.resolve(self._store, node_inputs)
        self.assertEqual(result, [
            {'x1': x1_mock, 'x2': x2_mock, 'x3': x3_mock, 'x4': x4_expected}
        ])
