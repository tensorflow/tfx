...
def resolve_single_channel(
  ...
  handle = mlmd_cm.get_handle(handle_like)
  config = metadata_pb2.MLMDServiceConfig()
  if channel.HasField('metadata_connection_config'):
    if not isinstance(handle_like, mlmd_cm.MLMDConnectionManager):
      raise RuntimeError(
          'The channel has the field metadata_connection_config, but the '
          'handle_like is not MLMDConnectionManager.')
    mlmd_connection_manager = cast(mlmd_cm.MLMDConnectionManager, handle_like)
