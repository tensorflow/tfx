...
def get_cache_context(
  ...
  h = hashlib.sha256()
  h.update(
      tf.compat.as_bytes(pipeline_info.SerializeToString(deterministic=True)))
  h.update(
      tf.compat.as_bytes(
          pipeline_node.executor.SerializeToString(deterministic=True)))
  ...
  for key, value in sorted(parameters.items()):
    h.update(tf.compat.as_bytes(key))
    h.update(tf.compat.as_bytes(tf.compat.as_str_any(value)))
