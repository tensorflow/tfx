...
def _transform_artifacts(artifacts: list[_TfxArtifact], type_hint: Any) -> Any:
  ...
  # Optional[T]
  is_opt, artifact_type = pure_typing_utils.maybe_unwrap_optional(type_hint)
  if is_opt and _is_valid_artifact_type(artifact_type):
  ...
  # Just T
  if _is_valid_artifact_type(artifact_type):
    artifacts = _deserialize_artifact(artifact_type, artifacts)
    if len(artifacts) == 1:
      return artifacts[0]
    else:
  ...
  def make_factory(self, name: str, type_hint: Any) -> Callable[..., Any]:
    def provide(exec_info: data_types.ExecutionInfo):
      if name in exec_info.input_dict:
        ...
        result = _transform_artifacts(exec_info.input_dict[name], type_hint)
