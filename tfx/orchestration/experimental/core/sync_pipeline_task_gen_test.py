...
class SyncPipelineTaskGeneratorTest(test_utils.TfxTest, parameterized.TestCase):
  ...
  def test_conditional_execution(self, evaluate):
    ...
    # Verify that the node state reflects whether or not the node was skipped.
    with self._mlmd_connection as m:
      pipeline_state = test_utils.get_or_create_pipeline_state(
          m, self._pipeline)
      with pipeline_state:
        eval_node_uid = task_lib.NodeUid.from_pipeline_node(
            self._pipeline, self._evaluator)
        eval_node_state = pipeline_state.get_node_state(eval_node_uid)
        if evaluate:
          self.assertFalse(
              eval_node_state.is_tagged_with(
                  pstate.NodeState.Tag.NODE_EXEC_SKIPPED))
        else:
          self.assertTrue(
              eval_node_state.is_tagged_with(
                  pstate.NodeState.Tag.NODE_EXEC_SKIPPED))
