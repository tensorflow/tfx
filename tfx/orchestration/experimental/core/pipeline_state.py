...
class NodeState(json_utils.Jsonable):
  ...
  status_msg: str = ''
  last_updated: float = attr.ib(factory=lambda: time.time())  # pylint:disable=unnecessary-lambda
  ...
  def to_run_state_history(self) -> List[Tuple[run_state_pb2.RunState, float]]:
    run_state_history = []
    for state in self.state_history:
      run_state_history.append((
          ...
          state.create_time))
    # Include current state in state history.
    run_state_history.append((NodeState(
        state=self.state,
        status_code=self.status_code,
        status_msg=self.status_msg).to_run_state(), self.last_updated))
...
class PipelineView:
  ...
  def get_node_run_states_history(
      self) -> Dict[str, List[Tuple[run_state_pb2.RunState, float]]]:
