...
def _orchestrate_stop_initiated_pipeline(
  ...
  for node in pstate.get_all_pipeline_nodes(pipeline):
    if service_job_manager and service_job_manager.is_pure_service_node(
...
def _orchestrate_active_pipeline(mlmd_handle: metadata.Metadata,
                                 task_queue: tq.TaskQueue,
                                 service_job_manager: Optional[
                                     service_jobs.ServiceJobManager],
                                 pipeline_state: pstate.PipelineState) -> None:
  ...
  exec_node_tasks = []
  finalize_pipeline_tasks = []
  finalize_node_tasks = []
  for task in tasks:
    if task_lib.is_exec_node_task(task):
      task = typing.cast(task_lib.ExecNodeTask, task)
      exec_node_tasks.append(task)
    elif task_lib.is_finalize_pipeline_task(task):
      task = typing.cast(task_lib.FinalizePipelineTask, task)
      finalize_pipeline_tasks.append(task)
    else:
      assert task_lib.is_finalize_node_task(task)
      task = typing.cast(task_lib.FinalizeNodeTask, task)
      finalize_node_tasks.append(task)

  if finalize_pipeline_tasks:
    assert pipeline.execution_mode == pipeline_pb2.Pipeline.SYNC
    assert len(finalize_pipeline_tasks) == 1
    assert len(exec_node_tasks) == len(finalize_node_tasks) == 0
    task = finalize_pipeline_tasks[0]
...
def _maybe_enqueue_cancellation_task(mlmd_handle: metadata.Metadata,
  ...
  """
  logging.info('XXXX: %s', node.node_info.id)
