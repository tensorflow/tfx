...
def revive_pipeline_run(
  ...
  with pstate.PipelineState.load_run(
    ...
    if (
        not env.get_env().concurrent_pipeline_runs_enabled()
        and (all_active := pstate.PipelineState.load_all_active(mlmd_handle))
        and all_active
    ):
      raise status_lib.StatusNotOkError(
          code=status_lib.Code.INVALID_ARGUMENT,
    ...
    if (
      ...
      logging.info('Tring to update during revive...')
