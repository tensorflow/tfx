...
class UserModuleFilePipDependency(base_component._PipDependencyFuture):  # pylint: disable=protected-access
  ...
  def resolve(self, pipeline_root: Text):
    ...
    if module_file and not fileio.exists(module_file):
      logging.warning(
          'Could not access module file %r for component %s; '
          'skipping Python user module wheel packaging.',
          module_file, self.component)
      return None
    if pipeline_root and not fileio.exists(pipeline_root):
      logging.warning(
          'Could not access pipeline root %r for component %s; '
          'skipping Python user module wheel packaging.',
          module_file, self.component)
      return None
    ...
    # module path key.
    self.component.spec.exec_properties[self.module_path_key] = user_module_key
