...
def should_package_user_modules():
  ...
  return True  # We should always package user modules into wheels.
...
class UserModuleFilePipDependency(base_component._PipDependencyFuture):  # pylint: disable=protected-access
  ...
  def __init__(self, module_file: Text, component: base_component.BaseComponent,
               exec_property_key: Text):
    self.module_file = module_file
    self.component = component
    self.exec_property_key = exec_property_key

  def resolve(self):
    # Package the given user module file as a Python wheel.
    dist_file_path, user_module_key = package_user_module_file(
        self.component.id, self.module_file)
    # Set the user module key to point to a module in this wheel.
    self.component.spec.exec_properties[
        self.exec_property_key] = user_module_key
...
def add_user_module_dependency(component: base_component.BaseComponent,
  ...
  dependency = UserModuleFilePipDependency(module_file, component,
                                           exec_property_key)
  component._add_pip_dependency(dependency)  # pylint: disable=protected-access
  # The following value is arbitrary and is only present before pipeline
  # execution, after which it is replaced by
  # `UserModuleFilePipDependency.resolve()`.
  component.spec.exec_properties[exec_property_key] = (
      'future:generate_pip_package(%s)' % module_file)
...
def _get_version_hash(user_module_dir: Text, source_files: List[Text]) -> Text:
  ...
  for source_file in source_files:
    ...
    file_contents = open(os.path.join(user_module_dir, source_file),
                         'rb').read()
...
def package_user_module_file(instance_name: Text,
                             module_path: Text) -> Tuple[Text, Text]:
  ...
  temp_dist_file_path = os.path.join(dist_dir, dist_files[0])
  os.remove(setup_py_path)
  # Copy wheel file to current directory.
  # TODO(ccy): Consider adding the notion of a staging directory for pipeline
  # resources instead of using the current directory.
  dist_file_path = dist_files[0]
  shutil.copyfile(temp_dist_file_path, dist_file_path)
  logging.info(
