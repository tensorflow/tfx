...
class ComponentSpec(with_metaclass(abc.ABCMeta, object)):
  ...
  def _validate_spec(self):
    ...
    for param_name, param in [
        ('PARAMETERS', self.PARAMETERS), ('INPUTS', self.INPUTS),
    ...
    for arg_name, arg in itertools.chain(
      ...
      if not isinstance(arg, _ComponentParameter):
        raise ValueError((
            ...
            'got %s (for argument %s) instead.') % (self.__class__, arg,
                                                    arg_name))
  ...
  def _parse_parameters(self):
    ...
    for arg_name, arg in itertools.chain(
      ...
      if arg_name not in unparsed_args:
        if arg.optional:
          ...
          raise ValueError(
              'Missing argument %r to %s.' % (arg_name, self.__class__))
    ...
    for arg_name, arg in self.PARAMETERS.items():
      ...
      if (inspect.isclass(arg.type)
          and issubclass(arg.type, message.Message)
          and value):
...
class BaseComponent(with_metaclass(abc.ABCMeta, object)):
  ...
  def __init__(
      self,
      spec: ComponentSpec,
      custom_executor_class: Optional[Type[
    ...
    if custom_executor_class:
      if not issubclass(custom_executor_class, base_executor.BaseExecutor):
        raise TypeError((
            ...
            'base_executor.BaseExecutor') % (custom_executor_class,
                                             self.__class__))
    self.executor_class = (
  ...
  def _validate_component_class(cls):
    ...
    if not (inspect.isclass(cls.SPEC_CLASS)
            and issubclass(cls.SPEC_CLASS, ComponentSpec)):
      raise TypeError((
          'Component class %s expects SPEC_CLASS property to be a subclass '
          'of base_component.ComponentSpec; got %s instead.') %
                      (cls, cls.SPEC_CLASS))
    if not (inspect.isclass(cls.EXECUTOR_CLASS)
            and issubclass(cls.EXECUTOR_CLASS, base_executor.BaseExecutor)):
      raise TypeError((
  ...
  def _validate_spec(self, spec):
    ...
    if not isinstance(spec, ComponentSpec):
      raise ValueError((
          ...
          'instance of ComponentSpec, got %s instead.') % (self.__class__,
                                                           spec))
    if not isinstance(spec, self.__class__.SPEC_CLASS):
      raise ValueError((
          '%s expects the "spec" argument to be an instance of %s; '
          'got %s instead.') % (self.__class__, self.__class__.SPEC_CLASS,
                                spec))
