...
class BaseComponent(with_metaclass(abc.ABCMeta, object)):
  ...
  def __init__(
    ...
    Args:
      ...
      label: Optional uniquely-identifying label for this instance of the
  ...
  def __repr__(self):
    return ('%s(spec: %s, executor_class: %s, driver_class: %s, label: %s, '
            'inputs: %s, outputs: %s)') % (
                self.__class__.__name__, self.spec, self.executor_class,
                self.driver_class, self.label, self.inputs, self.outputs)
  ...
  def component_id(self):
    ...
    """
    if self.label:
      return '{}.{}'.format(self.component_name, self.label)
    else:
