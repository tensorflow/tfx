...
from tfx.components.util import test_utils
...
class FactoryTest(test_utils.ComponentTestMixin, tf.test.TestCase):

  def setUp(self):
    super(FactoryTest, self).setUp()

    self.model = mock.Mock(standard_artifacts.Model)
    self.model.uri = os.path.join(self.testdata_dir, 'trainer', 'current')
