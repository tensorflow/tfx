...
class ExecutorTest(tf.test.TestCase):
  ...
  def setUp(self):
    ...
    self._schema = [('i', 'integer', None, None, None, None, None),
  ...
  def testPrestoToExample(self, mock_connection):
    ...
    mock_connection.return_value.cursor.return_value.fetchone.return_value = self._rows[
        0]
    ...
    with beam.Pipeline() as pipeline:
      examples = (
          pipeline | 'ToTFExample' >> executor._PrestoToExample(
              input_dict={},
              exec_properties={'connection_info': self._connection_info},
