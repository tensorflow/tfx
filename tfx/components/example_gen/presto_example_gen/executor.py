...
def _get_schema_dict(cursor: prestodb.dbapi.Cursor):  # pylint: disable=invalid-name
...
class _PrestoConverter(object):
  ...
  def __init__(self, query: Text, db_conn: prestodb.dbapi.Connection):
    ...
    try:
      ...
      cur.fetchone()
      self._type_map = _get_schema_dict(cur)
...
def _ReadFromPresto(  # pylint: disable=invalid-name, missing-docstring
...
def _PrestoToExample(  # pylint: disable=invalid-name
  ...
  return (pipeline
          | 'QueryTable' >> _ReadFromPresto(split_pattern, db_cursor)
