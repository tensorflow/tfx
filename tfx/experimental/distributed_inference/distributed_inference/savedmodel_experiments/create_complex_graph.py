...
def remote_op_b(input_ids1, input_ids2):
  ...
  def remote_lookup(input_ids1, input_ids2):
    with create_session(graph_b) as sess:
      return sess.run(result_b,
                      feed_dict={ids_b1: input_ids1, ids_b2: input_ids2})
...
def save_examples_as_graphdefs(export_dir):
  """Save the example graphs in the form of GraphDefs."""
...
def save_examples_as_savedmodels(export_dir):
  """Save the example graphs in the form of SavedModels."""
  with tf.compat.v1.Session(graph=graph_a) as sess:
    tf.compat.v1.saved_model.simple_save(
        sess,
        os.path.join(export_dir, 'graph_a'),
        inputs={'input': ids_a},
        outputs={'output': result_a})

  with tf.compat.v1.Session(graph=graph_b) as sess:
    tf.compat.v1.saved_model.simple_save(
        sess,
        os.path.join(export_dir, 'graph_b'),
        inputs={'input': ids_b1, 'input_1': ids_b2},
        outputs={'output': result_b})

  with tf.compat.v1.Session(graph=main_graph) as sess:
    tf.compat.v1.saved_model.simple_save(
        sess,
        os.path.join(export_dir, 'main_graph'),
        inputs={'input': ids1, 'input_1': ids2},
        outputs={'output': main_result})

if __name__ == "__main__":
  save_examples_as_savedmodels('./savedmodels')
