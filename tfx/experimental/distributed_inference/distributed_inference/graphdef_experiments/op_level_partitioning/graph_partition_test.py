...
class RelationTest(tf.test.TestCase):
  ...
  def test_layers(self):
    ...
    node_name_to_input_names = {'a1': [], 'a2': [], 'b1': ['a1'],
                                'b2': ['a1', 'a2'], 'c1': ['b1'],
                                'c2': ['b1', 'a1', 'b2', 'a2']}
    relations = graph_partition.Relations(node_name_to_input_names)

    self.assertFalse(relations.check_if_finished())
    self.assertFalse(relations.check_if_finished())

    self.assertIn(relations.get_next_node_name(), {'a1', 'a2'})
    self.assertIn(relations.get_next_node_name(), {'a1', 'a2'})
    self.assertFalse(relations.check_if_finished())
    self.assertIn(relations.get_next_node_name(), {'b1', 'b2'})
    self.assertIn(relations.get_next_node_name(), {'b1', 'b2'})
    self.assertFalse(relations.check_if_finished())
    self.assertIn(relations.get_next_node_name(), {'c1', 'c2'})
    self.assertIn(relations.get_next_node_name(), {'c1', 'c2'})
...
class PartitionTest(tf.test.TestCase):
  ...
  def test_op_names(self):
    """Validate op names."""
    op_keys = set(op_to_filename.keys())
    ...
    for execution_specs in op_to_execution_specs.values():
      for spec in execution_specs:
        if not spec['is_remote_op']:
          ...
          self.assertEqual(all_nodes, spec['inputs'].union(spec['outputs']))
