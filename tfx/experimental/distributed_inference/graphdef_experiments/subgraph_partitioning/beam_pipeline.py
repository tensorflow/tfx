...
def ExecuteOneGraph(pcoll,  # pylint: disable=invalid-name
  ...
  Args:
    pcoll: input PCollection, each unit contains
           {graph_name: {computed node: value}}
  ...
  for spec in execution_specs:
    if not spec['is_remote_op']:
      ...
      # remote op is the only output
      remote_graph_name = list(spec['outputs'])[0]

      count += 1
      pcoll = pcoll | str(count) >> _LoadRemoteGraphInputs(
          current_graph_name,
          remote_graph_name,
          op_to_remote_op_name_mapping)
      ...
      count += 1
      pcoll = pcoll | str(count) >> ExecuteOneGraph(
          op_to_execution_specs,
          op_to_remote_op_name_mapping,
          remote_graph_name)

      count += 1
      pcoll = pcoll | str(count) >> _ExtractRemoteGraphOutput(
          current_graph_name,
          remote_graph_name,
          op_to_execution_specs)
...
class _ExecuteOneSubgraph(beam.DoFn):
  ...
  def process(self, element, spec, graph_name):
    ...
    Args:
      ...
      spec: an execution spec describing a graph we want to execute:
  ...
  def _get_output_names(self, spec):
    return [_import_tensor_name(output_name)
...
def _LoadRemoteGraphInputs(pcoll,  # pylint: disable=invalid-name
  ...
  current_op_type = _get_op_type(current_graph_name,
                                 set(op_to_remote_op_name_mapping.keys()))
  mapping = op_to_remote_op_name_mapping[current_op_type][remote_graph_name]

  count = 0
  for placeholder_name, input_name in mapping.items():
    count += 1
    pcoll = pcoll | str(count) >> beam.Map(
        _copy_tensor,
        current_graph_name,
...
def _ExtractRemoteGraphOutput(pcoll,  # pylint: disable=invalid-name
  ...
  remote_graph_type = _get_op_type(remote_graph_name,
                                   set(op_to_execution_specs.keys()))
  remote_graph_specs = op_to_execution_specs[remote_graph_type]
  remote_graph_output_name = list(remote_graph_specs[-1]['outputs'])[0]
