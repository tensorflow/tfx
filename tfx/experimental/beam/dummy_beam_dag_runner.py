...
from tfx.experimental.config import dummy_pipeline_config
...
class DummyBeamDagRunner(tfx_runner.TfxRunner):
  """Tfx runner on Beam."""

  def __init__(self,
               beam_orchestrator_args: Optional[List[Text]] = None,
               config: Optional[dummy_pipeline_config.PipelineConfig] = None):
    ...
    """
    if config is None:
      config = dummy_pipeline_config.PipelineConfig(
    ...
    super(DummyBeamDagRunner, self).__init__(config)
