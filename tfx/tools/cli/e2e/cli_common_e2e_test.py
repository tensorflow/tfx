...
class CliCommonEndToEndTest(tf.test.TestCase):
  ...
  def testMissingRequiredFlag(self):
    ...
    result = self.runner.invoke(cli_group,
                                ['pipeline', 'create', '--engine', 'beam'])
    self.assertIn('CLI', result.output)
    self.assertIn("Missing option '--pipeline_path' / '--pipeline-path'",
                  result.output)
    ...
    self.assertIn('CLI', result.output)
    self.assertIn("Missing option '--pipeline_name' / '--pipeline-name'",
                  result.output)
...
#    self.assertIn("Missing option '--run_id' / '--run-id'", result.output)
