...
class TaxiContinuousPipelineBeamEndToEndTest(tf.test.TestCase):

  def setUp(self):
    super(TaxiContinuousPipelineBeamEndToEndTest, self).setUp()
    self._test_dir = os.path.join(
        os.environ.get('TEST_UNDECLARED_OUTPUTS_DIR', self.get_temp_dir()),
        self._testMethodName)

    self._pipeline_name = 'continuous_beam_test'
  ...
  def testTaxiContinuousPipelineResolver(self):
    ...
    for i in range(self._window_size):
      # TODO(jjma): Move this copy to setup, and use RangeConfig to ingest
      # specific spans once PR has been merged.
      io_utils.copy_file(os.path.join(self._init_data_root, 'data.csv'),
