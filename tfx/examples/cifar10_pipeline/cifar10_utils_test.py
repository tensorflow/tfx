...
"""Tests for tfx.examples.cifar10_pipeline.cifar10_utils."""
...
from tfx.examples.cifar10_pipeline import cifar10_utils
from tfx.utils import io_utils
from tfx.utils import path_utils


class Cifar10UtilsTest(tf.test.TestCase):

    def setUp(self):
        super(Cifar10UtilsTest, self).setUp()
    ...
    def testUtils(self):
        key = 'label'
        xfm_key = cifar10_utils._transformed_name(key)
        self.assertEqual(xfm_key, 'label_xf')

    def testPreprocessingFn(self):
        schema_file = os.path.join(self._testdata_path, 'schema_gen/schema.pbtxt')
        schema = io_utils.parse_pbtxt_file(schema_file, schema_pb2.Schema())
        feature_spec = cifar10_utils._get_raw_feature_spec(schema)
        working_dir = self.get_temp_dir()
        transform_output_path = os.path.join(working_dir, 'transform_output')
        transformed_examples_path = os.path.join(working_dir, 'transformed_examples')

        legacy_metadata = dataset_metadata.DatasetMetadata(dataset_schema.from_feature_spec(feature_spec))
            ...
            with tft_beam.Context(temp_dir=os.path.join(working_dir, 'tmp')):
                examples = (
                    ...
                    | 'ReadTrainData' >> beam.io.ReadFromTFRecord(
                        os.path.join(self._testdata_path, 'import_example_gen/train/*'),
                ...
                (transformed_examples, transformed_metadata), transform_fn = (
                        ...
                        | 'AnalyzeAndTransform' >> tft_beam.AnalyzeAndTransformDataset(cifar10_utils.preprocessing_fn))
...
def testTrainerFn(self):
    ...
    schema = io_utils.parse_pbtxt_file(schema_file, schema_pb2.Schema())
    training_spec = cifar10_utils.trainer_fn(hparams, schema)
