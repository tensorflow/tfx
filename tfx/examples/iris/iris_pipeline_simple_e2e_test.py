...
from tfx.examples.iris import iris_pipeline_simple
from tfx.orchestration import metadata
from tfx.orchestration.beam.beam_dag_runner import BeamDagRunner


class IrisPipelineSimpleEndToEndTest(tf.test.TestCase):

  def setUp(self):
    super(IrisPipelineSimpleEndToEndTest, self).setUp()
    self._test_dir = os.path.join(
        os.environ.get('TEST_UNDECLARED_OUTPUTS_DIR', self.get_temp_dir()),
        self._testMethodName)

    self._pipeline_name = 'test'
    self._data_root = os.path.join(os.path.dirname(__file__), 'data')
    self._module_file = os.path.join(
        os.path.dirname(__file__), 'iris_utils_simple.py')
  ...
  def testIrisPipelineSimple(self):
    pipeline = iris_pipeline_simple._create_pipeline(
