...
# ==============================================================================
"""Writes TFLite metadata and label file to the image classification models.

We need this for integrating custom TFLite models with MLKit
TODO: add this into tfx pipeline
"""
...
def define_flags():
  ...
  flags.DEFINE_string("export_model_name", None,
                      "Name of the exported model.")
  flags.DEFINE_string("author", None,
                      "Path to save the TFLite model files with metadata.")
  flags.DEFINE_boolean("export_json", True,
                       "Whether to also export the metadata in json format.")
  flags.mark_flag_as_required("model_path")
  flags.mark_flag_as_required("label_file")
  flags.mark_flag_as_required("export_directory")
  flags.mark_flag_as_required("export_model_name")

ModelSpecificInfo = collections.namedtuple('ModelSpecificInfo', [
    'name',
    'version',
    'image_width',
    'image_height',
...
class MetadataPopulatorForImageClassifier(object):
  ...
  def _create_metadata(self):
    ...
    model_meta.license = ("Apache License. Version 2.0 "
                          "http://www.apache.org/licenses/LICENSE-2.0.")
    if FLAGS.author:
      model_meta.author = FLAGS.author
    ...
    input_meta.description = (
        ...
        "tensor is a float between {2} and {3}.".format(
...
def main(_):
  model_file = FLAGS.model_path
  export_model_path = os.path.join(FLAGS.export_directory,
                                   FLAGS.export_model_name)
  if export_model_path == model_file:
    raise ValueError("Export model path is the same as input model path")
  ...
  populator = MetadataPopulatorForImageClassifier(
      export_model_path, _MODEL_INFO, FLAGS.label_file)
  populator.populate()

  logging.info("Finished populating metadata and associated file to the model:")
  logging.info(model_file)

  # Validate the output model file by reading the metadata
  displayer = _metadata.MetadataDisplayer.with_model_file(export_model_path)

  # Produce a json file with the metadata under the export path
  if FLAGS.export_json:
    export_json_file = os.path.join(
        FLAGS.export_directory,
        os.path.splitext(FLAGS.export_model_name)[0] + ".json")
    json_file = displayer.get_metadata_json()
    with open(export_json_file, "w") as f:
      f.write(json_file)

    logging.info("The metadata json file has been saved to:")
    logging.info(export_json_file)

  logging.info("The associated file that has been been packed to the model is:")
