...
def main(argv):
  ...
  Args:
    argv: Unparsed arguments for run_executor.py
      ...
      --exec_properties: JSON serialized dict of (non artifact) execution properties. Technically all the exec_properties values should be a primitive, and nested exec_properties needs to be JSON-encoded as a string. But as a convenience, the script allows you to feed in non-serialized values of exec_properties, which are then automatically serialized. If the execution properties need to be base64-encoded, use --exec_properties-base64 instead.
