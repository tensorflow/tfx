...
"""Contains the ModelBase class - the base class for serializable structures."""
...
def verify_object_against_type(x: Any, typ: Type[T]) -> T:
  ...
  # TODO(avolkov): Merge with parse_object_from_struct_based_on_type
  # which has almost the same code
  if typ is type(None):
    if x is None:
      return x
    else:
  ...
  if typ is Any or type(typ) is TypeVar:  # pylint: disable=unidiomatic-typecheck
    return x
  ...
  if hasattr(typ, '__origin__'):  # Handling generic types
    if typ.__origin__ is Union:  # Optional == Union
      ...
      # Shortcut for Optional[] tests.
      # Can be removed, but the exceptions will be more noisy.
      if None in possible_types and x is None:
        return x
      ...
      exception_lines.append(
          'Error: Object "{}" is incompatible with type "{}".'.format(x, typ))
    ...
    # In python <3.7 typing.List.__origin__ == None;
    # Python 3.7 has working __origin__, but no __extra__
    # TODO(avolkov): Remove the __extra__ once we move to Python 3.7
    generic_type = typ.__origin__ or getattr(
        typ, '__extra__', None
    )

    if generic_type in [
        list, List, abc.Sequence, abc.MutableSequence, Sequence, MutableSequence
    ] and not isinstance(x, str):  # ! str is also Sequence
      if not isinstance(x, generic_type):
        raise TypeError(
            'Error: Object "{}" is incompatible with type "{}"'.format(x, typ))
      type_args = typ.__args__ if typ.__args__ is not None else (
          Any,
          Any)  # Workaround for Python <3.7 (where Mapping.__args__ is None)
      ...
      return x
        ...
        dict, Dict, abc.Mapping, abc.MutableMapping, Mapping, MutableMapping,
        OrderedDict
    ]:
      if not isinstance(x, generic_type):
        raise TypeError(
...
def parse_object_from_struct_based_on_type(struct: Any, typ: Type[T]) -> T:
  ...
  Supports list and dict types from the typing package plus Optional[] and
  Union[] types.
    If some type is a class that has .from_dict class method, that method is
    used for object construction.

  Args:
    struct: structure to parse
    typ: desired object type
  Returns:
    Materialized object
  """
  if typ is type(None):
  ...
  try:  # isinstance can fail for generics
    ...
    #     #! str is also Sequence
    #     and not (typ is Sequence and type(struct) is str)
    #     #! bool is int
    #     and not (typ is int and type(struct) is bool)
    # ):
    if type(struct) is typ:  # pylint: disable=unidiomatic-typecheck
      return struct
  except TypeError:
    pass
  ...
  if hasattr(typ, '__origin__'):  # Handling generic types
    if typ.__origin__ is Union:  # Optional == Union
      ...
      # # Shortcut for Optional[] tests.
      # # Can be removed, but the exceptions will be more noisy.
      # if type(None) in possible_types and struct is None:
      #     return None
      # Hack for Python <3.7 which for some reason
      # "simplifies" Union[bool, int, ...] to just Union[int, ...]
    ...
    if struct is None:
      raise TypeError(
          'Error: None structure is incompatible with type {}'.format(typ))
    ...
    if generic_type in [
        list, List, abc.Sequence, abc.MutableSequence, Sequence, MutableSequence
    ] and type(struct) is not str:  # pylint: disable=unidiomatic-typecheck
      if not isinstance(struct, generic_type):
        raise TypeError(
            'Error: Structure "{}" is incompatible with type "{}" - it does not have list type.'
            .format(struct, typ))
      # Workaround for Python <3.7 (where Mapping.__args__ is None)
      type_args = typ.__args__ if typ.__args__ is not None else (Any, Any)
      inner_type = type_args[0]
      return [
    ...
    # In Python <3.7 abc.Mapping and typing.Mapping are different
    elif generic_type in [
        dict, Dict, abc.Mapping, abc.MutableMapping, Mapping, MutableMapping,
        OrderedDict
    ]:
...
def convert_object_to_struct(
    obj,
    serialized_names: Mapping[str, str] = None,
):
  """Converts an object to structure (usually a dict).

    Serializes all properties that do not start with underscores.
    If the type of some property is a class that has .to_dict class method, that
    method is used for conversion.
    Used by the ModelBase class.

  Args:
    obj: Object that needs to be converted to a simple structure
    serialized_names: specifies the mapping between __init__ parameter names
      and the structure key names for cases where these names are different
      (due to language syntax clashes or style differences).
  Returns:
    Constructed object
  """
  serialized_names = serialized_names or {}
  signature = inspect.signature(obj.__init__)  # Needed for default values
  result = {}
  # TODO(avolkov): Make it possible to specify the field ordering
  # regardless of the presence of default values
  for python_name in signature.parameters:
...
def parse_object_from_struct_based_on_class_init(
  ...
  Converts all constructor arguments to appropriate types based on the
  __init__ type hints.
  Used by the ModelBase class.

  Args:
    cls: Type of object to construct from the struct
    struct: Structure (dicts, lists and primitives) that should be parsed
    serialized_names: specifies the mapping between __init__ parameter names
      and the structure key names for cases where these names are different
      (due to language syntax clashes or style differences).
  Returns:
  ...
  parameter_types = get_type_hints(
      cls.__init__,
  )  # Properlty resolves forward references

  serialized_names_to_pythonic = {v: k for k, v in serialized_names.items()}
  # If a pythonic name has a different original name,
  # we forbid the pythonic name in the structure.
  # Otherwise, this function would accept "python-styled" structures
  # that should be invalid
  forbidden_struct_keys = set(serialized_names_to_pythonic.values()).difference(
...
class ModelBase:
  ...
  The object fields, their types and default values are taken from the
  __init__ method arguments.
  Override the _serialized_names mapping to control the key names of the
  serialized structures.

  The derived class objects will have the .from_dict and .to_dict methods for
  conversion to or from structure. The base class constructor accepts the
  arguments map, checks the argument types and sets the object field values.
  ...
  class TaskSpec(ModelBase):
      ...
      def __init__(self,
          ...
          is_enabled: Optional[Union[ArgumentType, Predicate] = None,
      ):
          super().__init__(locals()) #Calling the ModelBase constructor to
          check the argument types and set the object field values.

  task_dict = {
      'component_ref: {...},
      'arguments: {...},
  }
  task_spec = TaskSpec.from_dict(task_dict) # returns instance of TaskSpec
  task_struct = task_spec.to_dict() # Returns a dict equal to task_dict
  """
  _serialized_names = {}
  ...
  def __init__(self, args):
    ...
    field_values = {
        k: v for k, v in args.items() if k != 'self' and not k.startswith('_')
    }
  ...
  @classmethod
  def from_dict(cls: Type[T], struct: Mapping) -> T:  # pylint: disable=g-bare-generic
    return parse_object_from_struct_based_on_class_init(
        cls, struct, serialized_names=cls._serialized_names)

  def to_dict(self) -> Mapping:  # pylint: disable=g-bare-generic
