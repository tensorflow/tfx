  ...
  for name, arg_format in arg_formats.items():
    if arg_format == function_parser.ArgFormats.INPUT_ARTIFACT:
      ...
      if name in arg_defaults and arg_defaults[name] is not None:
        raise ValueError('beam Pipeline parameter does not accept default ',
...
def component(
  ...
  * `OutputArtifact[ArtifactType]`: indicates that an output artifact object of
  ...
  if use_beam and beam.Pipeline not in parameters.values():
    raise ValueError('The decorated function must have an optional parameter '
                     'of type BeamComponentParameter[beam.Pipeline] with '
                     'value None when use_beam=True.')
