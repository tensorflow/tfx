...
class SpansResolverTest(tf.test.TestCase):
  ...
  def testMergeMultipleChannels(self):
    with metadata.Metadata(connection_config=self._connection_config) as m:
      contexts = m.register_pipeline_contexts_if_not_exists(self._pipeline_info)
      ...
      # Simulate a second, different component execution.
      second_component_info = data_types.ComponentInfo(
          component_type='a.b.c',
          component_id='my_component2',
          pipeline_info=self._pipeline_info)
      ...
      m.register_execution(
          exec_properties={},
          pipeline_info=self._pipeline_info,
          component_info=second_component_info,
          contexts=contexts)
      m.publish_execution(
          component_info=second_component_info,
          output_artifacts={'key': [artifact_two]})

      # Resolve spans in range [1, 2], and merge all input channels into one.
      resolver = spans_resolver.SpansResolver(
          range_config=range_config_pb2.RangeConfig(
             static_range=range_config_pb2.StaticRange(
                 start_span_number=1, end_span_number=2)),
          merge_same_artifact_type=True)
      ...
      resolve_result = resolver.resolve(
          ...
          source_channels={
              ...
              'channel2':
                  types.Channel(
                      type=artifact_two.type,
                      producer_component_id=second_component_info.component_id,
                      output_key='key')
          })

      # Since we output Span-1 and Span-2, the final merged channel should
      # contain a merged result of 2 Spans, Span-1 and Span-2.
      output_key = standard_artifacts.Examples.TYPE_NAME
      self.assertTrue(resolve_result.has_complete_result)
      self.assertEqual([
          artifact.uri
          for artifact in resolve_result.per_key_resolve_result[output_key] 
      ], [artifact_one.uri, artifact_two.uri])
      self.assertTrue(resolve_result.per_key_resolve_state[output_key])
