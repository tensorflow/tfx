...
class ResolverFunction:
  ...
  Usage:
      ...
      rf.trace()
      rf.output_node  # Bar(Foo(INPUT_NODE, foo=1), bar='x')
  """

  def __init__(self, f: Callable[..., resolver_operator.OpNode]):
    self._function = f
    self._output_node = None

  @property
  def output_node(self) -> resolver_operator.OpNode:
  ...
  def trace(self):
    ...
    output_node = self._function(resolver_operator.OpNode.INPUT_NODE)
    if not isinstance(output_node, resolver_operator.OpNode):
      raise TypeError(
