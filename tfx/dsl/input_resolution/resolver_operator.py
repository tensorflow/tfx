...
import abc
import inspect
from typing import Dict, List, Any, Mapping, Generic, TypeVar, Type, Union, ClassVar
...
# TODO(b/188020544): Consider interoperability with
...
class _ResolverOpMeta(abc.ABCMeta):
  ...
  def __init__(cls, name, bases, attrs):
    ...
    cls._check_apply_method(attrs['apply'])
    super().__init__(name, bases, attrs)

  def _check_apply_method(cls, method):
    """Validate ResolverOp.apply method."""
    signature = inspect.signature(method)
    if signature.return_annotation is inspect.Signature.empty:
      raise TypeError('apply method should have return type annotation.')
    if signature.return_annotation != ArtifactMultimap:
      raise TypeError('Return type should be Dict[str, List[Artifact]].')
    params = list(signature.parameters.values())
    if len(params) != 2:
      raise TypeError('apply argument should be self and input_dict.')
    if params[1].annotation is inspect.Parameter.empty:
      raise TypeError('apply method should have argument type annotation.')
    if params[1].annotation != ArtifactMultimap:
      raise TypeError('Argument type should be Dict[str, List[Artifact]].')
  ...
  def create(cls, **props: Any) -> 'ResolverOp':
    ...
    """
    real_intance = super().__call__()
    cls._check_kwargs(props)
    for name, value in props.items():
      setattr(real_intance, name, value)
    return real_intance
...
class OpNode:
  """OpNode represnets a ResolverOp invocation."""
  ...
  arg = attr.ib()
  # ResolverOpProperty for the ResolverOp, given as a keyword arguments.
