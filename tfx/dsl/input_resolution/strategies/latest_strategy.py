...
class LatestStrategy(resolver.ResolverStrategy):
  """LatestStrategy resolves the latest n(=1) artifacts per each channel.

  Note that this ResolverStrategy is experimental and is subject to change in
  terms of both interface and implementation.

  Don't construct LatestStrategy directly, example usage:
  ```
    model_resolver = Resolver(
        instance_name='latest_model_resolver',
        strategy_class=LatestStrategy,
