...
class SpansStrategy(resolver.ResolverStrategy):
  """SpanStrategy resolves artifacts based on "span" property.

  Note that this ResolverStrategy is experimental and is subject to change in
  terms of both interface and implementation.

  Don't construct SpansStrategy directly, example usage:
  ```
    examples_resolver = Resolver(
        instance_name='span_resolver',
        strategy_class=SpansStrategy,
