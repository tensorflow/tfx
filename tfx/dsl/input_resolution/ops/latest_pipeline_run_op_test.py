...
class LatestPipelineRunTest(tf.test.TestCase):
  ...
  def setUp(self):
    ...
    context = resolver_op.Context(store=self._metadata.store)
  ...
  def testLatestCreateTime_Empty(self):
    actual = self._latest_pipeline_run_op.apply([])
  ...
  def testLatestCreateTime_NotEmpty(self):
    ...
    actual = self._latest_pipeline_run_op.apply(
        [input_artifact_1, input_artifact_2])
    self.assertAllEqual(actual, [input_artifact_2])
