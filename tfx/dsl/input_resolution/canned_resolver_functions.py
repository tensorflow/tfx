...
def pick(channel: channel_types.BaseChannel, i: int, /):
  ...
  Like in python, negative index would work as an index from the backward.

  If the value is out of index, in synchronous pipeline it raises an error and
...
def pick_range(  # pylint: disable=redefined-builtin
  ...
  """Pick range(start, stop) of the input artifacts.

  Like in python, negative index would work as an index from the backward.

  If the range is out of index, then like in python slice, the result would be
  truncated to the available values. You can use min_count to ensure the range
  has enough values. In synchronous pipeline, it is an error if the min_count is
  not met. In asynchronous pipeline, it will wait until min_count is met.
