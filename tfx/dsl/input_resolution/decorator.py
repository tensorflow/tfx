...
from tfx.dsl.components.common import resolver as rslv
...
def resolver(
    f: Callable[..., resolver_operator.OpNode]
) -> Callable[..., rslv.Resolver]:
  ...
  """
  @functools.wraps(f)
  def wrapper(**channels: tfx.types.Channel) -> rslv.Resolver:
    rf = resolver_function.ResolverFunction(f)
    rf.trace()
    return rslv.Resolver(function=rf, **channels)
