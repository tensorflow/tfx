...
class Compiler:
  ...
  def _compile_node(
    ...
    for dsl_context in context_manager.get_contexts(tfx_node):
      if isinstance(dsl_context, for_each.ForEachContext):
        for input_key, channel in tfx_node_inputs.items():
          if (isinstance(channel, for_each.SlicedChannel) and
              channel.wrapped is dsl_context.channel):
          ...
          raise ValueError(
              f"Unable to locate ForEach loop varable {dsl_context.channel} "
